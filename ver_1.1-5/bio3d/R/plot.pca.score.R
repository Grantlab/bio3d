"plot.pca.score" <-
function(x, start.row=1, end.row=nrow(x),
                           col=rainbow(nrow(x)),
                           lab="", ... ) {
  ...
  limits<-max(abs(c(range(x[,1]),range(x[,1]),range(x[,1]))))
  ...
  text(x[start.row:end.row,1]+text.offset,
       x[start.row:end.row,2]+text.offset,
       labels = labels)
  ...
  abline(h=0,col="gray",lty=2);abline(v=0,col="gray",lty=2)
  text(x[start.row:end.row,3]+text.offset,
       x[start.row:end.row,2]+text.offset,
       labels = labels)
  ...
  abline(h=0,col="gray",lty=2);abline(v=0,col="gray",lty=2)
  text(x[start.row:end.row,1]+text.offset,
       x[start.row:end.row,3]+text.offset,
       labels = labels)

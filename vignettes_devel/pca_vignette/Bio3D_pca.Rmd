...
plot.bio3d(bfacts, resno=resnos, sse=pdb, ylab="B-factor", xlab="Residue", typ="l") 
```
...
lets extract the sequence for the loop region (P-loop) between beta 1 
...
See `help(combine.sel)` for more details. 
...
Lets compare the Calpha atom based pseudo-torsion angles between two structures, the inactive (PDB ID 1TAG) and active (PDB ID 1TND) transducin alpha subunit:
...
plot( (a - b), nlevels=10, grid.col="gray", xlab="1tag", ylab="1tnd") 
```

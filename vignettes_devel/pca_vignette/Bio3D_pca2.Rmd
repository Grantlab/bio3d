...
plot(hc.rd, labels=pdbs$id, hang=-1, cex=0.5, ylab="RMSD (Å)", main="RMSD Cluster Dendrogram")
```

Or as a nicer plot:
```{r, fig.cap="RMSD clustering of transducin structures"}
...
plot(dend_colored, cex.axis=1.2, cex.lab=1.2, cex.main=1.2, ylab="RMSD (Å)", main="RMSD Cluster Dendrogram")
par(op)
``` 
...
```{r, results="hide", fig.cap="Difference of distance matrices between structures in GDP(1tag) and GTP(1tnd) nucleotide states"}
...
```{r, echo=FALSE, fig.cap="Contribution of each residue to the first three principal components"}
...
```{r, fig.cap="Clustering based on PC1-PC2"}
hc <- hclust(dist(pc.xray$z[,1:2]))
grps <- cutree(hc, h=30)
cols <- c("red", "green", "blue")[grps]
plot(pc.xray$z[, 1:2], typ="p", pch=16, col=cols, xlab="PC1", ylab="PC2", cex=1.2, cex.axis=1.5, cex.lab=1.5) 
...
plot(dend_colored, cex.axis=1.2, cex.lab=1.2, cex.main=1.2, ylab="Distance in PC Space", main="PC1-2")

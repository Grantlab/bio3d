---
title: "Integrated structural and evolutionary ensemble analysis with Bio3D"
...
```{r, fig.cap="Analysis of mode similarity between modes obtained from the *ANM* and *calpha* force fields by calculating mode overlap and root mean square inner product (RMSIP) with function **rmsip()**. An RMSIP value of *1* depicts identical directionality of the two mode subspaces. "}
...
```{r message=FALSE, fig.width=5, fig.height=5.5, fig.cap="Correlation map revealing correlated and anti-correlated regions in the protein structure."}
...
```{r width=7, fig.height=4, fig.cap="Overlap analysis between the modes of the open subunit and the conformational difference vector between the closed-open state."}
...
**GeoStaS with NMA**: Starting from the calculated normal modes, we generate
...
![Visualization of domain assignment obtained from function **geostas()** using (left) an ensemble of X-ray structures and (right) NMA.](figure/geostas.png)
...
```{r plot_enma1, fig.width=10, fig.height=5, fig.cap="Results of ensemble NMA on four distinct bacterial species of the DHFR enzyme."}
...
```{r  ex2A-plot_enma2, fig.cap="Residue cross-correlations for each DHFR structures analyzed."}
# Set DCCM plot panel names for combined figure
dimnames(cij$all.dccm)=list(NULL, NULL, ids)
plot.dccm(cij$all.dccm)
```

```{r ex2A-plot_enma3, fig.cap="Residue cross-correlations present in all DHFR structures analyzed."}
...
```{r ex2B-modes, cache=TRUE, warning=FALSE, message=FALSE}
# Calculate normal modes of the 53 structures
modes <- nma.pdbs(pdbs, ncore=4)
```

```{r ex2B-plot, fig.width=9, fig.height=5, fig.cap="Structural dynamics of transducin. The calculation is based on NMA of 53 structures: 28 GTP-bound (red), and 25 GDP-bound (green)."}
...
```{r ex4B-calphax, cache=TRUE, results="hide", warning=FALSE}

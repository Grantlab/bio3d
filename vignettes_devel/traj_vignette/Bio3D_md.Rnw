...
Note that to follow along with this vignette the \href{http://www.drive5.com/muscle/}{MUSCLE multiple sequence alignment program} must be installed on your system and in the search path for executables. Please see the installation vignette for full details.
...
\paragraph{Question:} How would you extract the amino acid sequence of the HIVpr system in one letter and 3-letter forms? HINT: try \texttt{help.search("PDB sequence")} for a Bio3D function that might help you.
...
<<echo=TRUE, fig.cap="Simple time series of RMSD from the initial structure (note periodic jumps that we will later see correspond to transient openings of the flap regions of HIVpr)">>=

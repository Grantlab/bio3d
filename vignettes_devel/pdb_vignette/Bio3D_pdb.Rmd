...
biological units via symmetry operations (e.g. rotation and translation) performed on the stored coordinates. 
...
#### Side-note: {.unnumbered}
The biological units described in a PDB file can be determined based on either expertise of authors of 
the crystallographic structure or calucaltion of software (See [here](http://www.rcsb.org/pdb/101/static101.do?p=education_discussion/Looking-at-Structures/bioassembly_tutorial.html) for more details).  
By default, **biounit()** returns all the determinations, which may contain biological units with various
number of chains (i.e. distinct multimeric state). Note that it is dependent on users which biological unit 
is finally adopted for further analysis. The 'names' attributes of the returned 'list' by 
**biounit()** provides basic information about how the biological unit was determined and the corresponding 
multimeric state, which may faciltate users to make the decision. 

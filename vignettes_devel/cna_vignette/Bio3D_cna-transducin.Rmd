---
title: "Integrating NMA with correlation network analysis in Bio3D"
...
This document provides **Supporting Information S4** for *Integrating protein structural dynamics and evolutionary analysis with Bio3D*. The vignette is generated with **Bio3D version `r packageVersion("bio3d")`**. 
...
```{r filter_dccm, cache=TRUE, eval=FALSE}
cij <- filter.dccm(cijs0, xyz=pdbs, fac=annotation[, "state3"], cutoff.cij=0.35, dcut=10, scut=0, pcut=0.75, ncore=8)
```

```{r nets, cache=TRUE, eval=FALSE}
...
```{r figure2, fig.cap="Correlation networks for GTP \"active\" and GDI \"inhibitory\" conformational states of transducin. Networks are derived from ensemble NMA of available GTP and GDI crystallographic structures.", echo=c(3,5,6,8), eval=FALSE}
layout(matrix(c(1:4), 2, 2))
par(mar=c(0.1, 0.1, 3, 0.1))

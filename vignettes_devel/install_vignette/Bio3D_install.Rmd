...
```{r, eval=FALSE}
install.packages("bio3d_2.2-0.tar.gz")
```
...
**DSSP** is neither available from the package managers, but can be installed by:
...
A minimal version of Bio3D with reduced functionaly (i.e. for reading/writing
...
based R system the installtion is similar to the Linux installation
described above:
...
We recommend using [MacPort](http://www.macports.org/) for the installtion of NetCDF
headers and libraries. For more details on the installation process see e.g.
<http://mazamascience.com/WorkingWithData/?p=1474>. After the installtion of MacPort,
NetCDF can be installed with the command:

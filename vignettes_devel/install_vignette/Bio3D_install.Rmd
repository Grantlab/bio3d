...
**install.packages()** at the R command prompt to install from your 
local [CRAN](http://cran.r-project.org/web/packages/bio3d/):

```{r, eval=FALSE}
install.packages("bio3d", dependencies=TRUE)
```

Alternatively, if you prefer to use [RStudio](https://www.rstudio.com/products/rstudio/), select `Tools` $\rightarrow$ `Install Packages..`, Enter **bio3d** in the `Packages` text box (and make sure `Install Dependencies` is selected). Then click `Install`.
...
for executables. Background details and install instructions for each of these packages on different operationg systems are provided further below. 
...
On a **Linux/UNIX** system you should use your appropriate package manager (e.g. *apt-get*, *yum*, etc.). If you experience problems with any of these steps please read on for alternative installation methods.
...
In addition, for full Bio3D functionality you should have *MUSCLE* (and optionally *DSSP*)
...
# Installing Bio3D
Installing required headers, libraries, R packages, and utility programs are
relatively straightforward on most common architectures and operative systems. Below
we provide installation instructions for some of the most common OS's. 
...
Either way start R/RStudio or the R.app and from the resulting R prompt install the Bio3D package from [CRAN](http://cran.r-project.org/web/packages/bio3d/) using the following command:
```{r, eval=FALSE}
install.packages("bio3d", dependencies=TRUE)
```

We recommend using [homebrew](http://brew.sh/index.html) for the installation of muscle, dssp and NetCDF headers and libraries. For information on homebrew see <http://brew.sh/index.html>. After the installation of homebrew,
...
In RStudio, select `Tools` $\rightarrow$ `Install Packages..`, Enter **bio3d** in the `Packages` text box, make sure `Dependencies` is selected and click `Install`.
...
For Red Hat based systems (e.g. Fedora) the equivalent command is:
    ...
    yum install R-base R-devel netcdf-devel netcdf libxml2-devel \
                libcurl-devel seaview pymol 

The Bio3D package can be obtained and installed via [CRAN](http://cran.r-project.org/web/packages/bio3d/). Start R by issuing the command `R` and then from the R prompt install the Bio3D package:
...
```{r, eval=FALSE}
install.packages("bio3d_3.0-0.tar.gz")
```
...
of time-saving customizations (see the [cheat

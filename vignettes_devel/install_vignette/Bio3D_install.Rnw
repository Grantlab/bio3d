...
\section{Background}
Bio3d is an R package that provides interactive tools for structural bioinformatics. The primary focus of bio3d is the analysis of biomolecular structure, sequence and trajectory data CITATION.
...
\subsection*{Whre can I find more information?}
The latest version of the package, full documentation and furhter vignettes can be obtained from the main bio3d website: \url{http://thegrantlab.org/bio3d/}. 
...
\subsection*{Addational R Packages}
Bio3D makes use of a number of additional R packages include filehash, bigmemory and ncdf. These can be most easily installed from within R with the command:
...
<<eval=FALSE>>=
install.packages("~/Downloads/bio3d_1.2.tar.gz", repos=NULL, type="source") 
@
...
\texttt{setenv R\_LIBS /net/home/bgrant/software/R/lib/R/library} 

\paragraph{}Obviously you will want to change the path above to a directory of your choice. 
...
Start R and from GUI click Packages and then Install Package(s) from local zip file then simply select your downloaded bio3 zip file and click Open to finish the installation. 

\section{Obtaining the development version of Bio3D}
...
Alternative instalation methods and addational instructions, including how to contribute to the bio3d project, are posted on the wiki section of our \href{https://bitbucket.org/Grantlab/bio3d/}{bitbucket repository}. 


\section{Addational utilities}
There are a number of additional packages and programs that will either interfae directly with Bio3D (MUSCLE, DSSP and STRIDE) or that we consider generally invaluable for working with biomolecular structure and sequence data (e.g. VMD and SEAVIEW) . A brief description of how to obtain these additional packages is given below.
...
After downloading MUSCLE, it should be unzipped and renamed to just "muscle" and placed in a directory such as "/usr/sbin/". 
...
\paragraph{DSSP:}
DSSP is another secondary structure analysis program available from a number of sources including \url{http://www.cmbi.ru.nl/dssp.html}. 
...
You should now be able to load the bio3d library into your current R session by typing the usual library(bio3d) command at the R Console. 

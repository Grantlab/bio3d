...
cluster.sim <- function(grp1, grp2, self=FALSE, minSZ=1, method="jaccard") {
  ...
  if(is.null(names(grp1)) || is.null(grp2) ) {
    warning("Input cluster membership vectors should have a names attribute, attempting to assign these")
    if( length(grp1)==length(grp2) ) {
      names(grp1) = 1:length(grp1); names(grp2) = 1:length(grp2)
    } else {
      stop("Input vectors of different lengths so we cant assign the same names")
    }
  }
  ## check if there are repetition in vector names
  if( any(c( duplicated( names(grp1)), duplicated( names(grp2)) )) ) {
    stop("Input cluster membership vectors contain duplicated names")
  }
  ## check if there is at least a couple of names overlapping
  if( sum(names(grp1) %in% names(grp2) ) < 2 ) {
    stop("Less than two overlapping element names in input vectors")
  }

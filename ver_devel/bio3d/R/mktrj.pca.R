"mktrj.pca" <- function(x=NULL,   # pca data structure
  ...
  plus  <- sapply(c(zcoor), scor, u=x$U[,pc], m=x$mean)
  minus <- sapply(c(-zcoor), scor, u=x$U[,pc], m=x$mean)

...
test_that("atom.select() gets correct selections with various options", {
  ...
  expect_equal(a.inds$atom[98], 171) 
  ...
  expect_equal(length(pro.inds) + length(npro.inds), nrow(pdb$atom) - 1) # omit ALT
  ...
  expect_equal(length(wat.inds) + length(nwat.inds), nrow(pdb$atom) - 1) # omit ALT
  ...
  expect_equal(length(h.inds) + length(noh.inds), nrow(pdb$atom) - 1) # omit ALT
  ...
  expect_equal(comb2.inds$atom, 136)
